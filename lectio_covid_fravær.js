...
const notes = [ // Notes are randomly picked from this list
    'Er til time',
    'Er ved pc',
    'Mødt',
    'Er her'
...
;(async function() {
    ...
    switch (argv[2]) {
        case 'post': {
            ...
            console.log(`Posting absense note... (${argv[4]})`)
            await post_absense_note(id,argv[3])
            ...
            if (safemode) {
                for (let s of schedule_for) {
                    setTimeout(async ()=>{
                        try {
                            ...
                            await post_absense_note(id,s.absid)
                            console.log(`Note posted.`)
                        } catch (e) {
                ...
                for (let s of schedule_for) {
                    ...
                    await create_post_task(
                        ...
                        `${s.team}, ${s.teacher_alias}, ${s.start}`
                    )
                    scheduled++
                }
...
async function post_absense_note(id,absid) {
    const validationData = await fetch(`https://www.lectio.dk/lectio/557/aktivitet/aktivitetforside2.aspx?absid=${absid}&elevid=${id}&lectab=elevindhold`)
    ...
    body.append('s$m$Content$Content$Elevindhold$tocAndToolbar$elevindholdEditLV$ctrl1$HomeworkEditLV$ctrl0$editor$ed',
        `<h1 id="321">${notes[Math.floor(Math.random()*notes.length)]}</h1>`
    )
    const data = await fetch(`https://www.lectio.dk/lectio/557/aktivitet/aktivitetforside2.aspx?absid=${absid}&elevid=${id}&lectab=elevindhold`,{
        method: 'POST',
        body
    })
    const $ = cheerio.load(await data.text())
...
async function create_post_task(absid,time,description) {
    const date = new Date(time)
    await exec(`schtasks /create /sc once /tn "NodeTasks\\${absid.toString().replace('"','\\"')}" /sd ${
    ...
    } /tr "'${process.argv0}' '${process.argv[1]}' keepopen post ${absid} '${description.replace('"','\\"')}'"`)
}
